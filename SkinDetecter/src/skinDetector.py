...
class SkinDetector():
	...
	def __color_segmentation(self) -> None:
		...
		lower_HSV_values = np.array([0, 15, 0], dtype="uint8")   #0, 15, 0
		upper_HSV_values = np.array([15, 255, 250], dtype="uint8")  #15, 255, 250

		lower_YCbCr_values = np.array((0, 150, 107), dtype="uint8") #0, 139, 108
		upper_YCbCr_values = np.array((255,188,126), dtype="uint8") #255,189,136
	...
	def show_all_images(self, window_title="Result") -> None:
		...
		cv2.imshow(window_title, self.skin)
		cv2.imwrite('D:/skinImage.jpg', self.skin)
		cv2.waitKey(0)
